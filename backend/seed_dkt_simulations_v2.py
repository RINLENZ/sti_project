...
PROFILS = [
    {
        ...
        "proba_2_macro":     0.20,               # 20% des sessions visitent 2 macro-KCs
    },
    {
    ...
    {
        "label":             "volatil",          # *** profil clé pour DKT-E ***
        "weight":            0.30,
        "p_init":            (0.18, 0.06),
        "p_learn":           (0.22, 0.06),
        "p_slip":            (0.15, 0.04),
        "p_guess":           (0.22, 0.04),
        "humeur_base":       (0.55, 0.20),       # forte variabilité ENTRE sessions
        "humeur_drift":      0.08,               # forte dérive AU SEIN d'une session
        "n_sessions":        (15, 25),
        "exos_par_session":  (5, 10),
        "proba_2_macro":     0.25,
    },
...
def proba_reussite_avec_humeur(p_mastery, humeur, params_bkt):
    """
    P(correct) = P(mastery) × (1 - P_slip_ajusté)  +  (1 - P(mastery)) × P_guess

    Le P_slip dépend de l'humeur courante :
      - humeur = 0.5  →  P_slip nominal
      - humeur = 0.2  →  P_slip + 0.10  (apprenant distrait → plus d'erreurs)
      - humeur = 0.8  →  P_slip - 0.06  (apprenant concentré → moins d'erreurs)

    C'est la composante CLÉ : l'humeur (donc l'engagement qui la reflète)
    contient une information prédictive sur la performance future qui
    n'est pas dans la performance passée.
    """
    ajustement = (0.5 - humeur) * 0.30
    p_slip_eff = max(0.01, min(0.50, params_bkt["P_slip"] + ajustement))
...
def simuler_engagement(humeur, correct, taux_recent, exo_position, n_total):
    """
    Les 4 scores d'engagement reflètent principalement l'HUMEUR du moment,
    plus une contribution modérée de la performance récente.

    Pondération clé :
      - facial    : 80% humeur + 20% performance courante (visible sur le visage)
      - audio     : 70% humeur + 30% bruit (parfois pas de voix → fallback dispo)
      - behavioral: 60% humeur + 40% taux de réussite récent (idle/help/retry)
      - fused     : combinaison pondérée 0.4·facial + 0.3·audio + 0.3·behavioral

    Le score reste dans [0.05, 0.95] (jamais d'extrême parfait).
    """
    ...
    facial     = max(0.05, min(0.95, 0.80 * humeur + 0.20 * perf_signal     + random.gauss(0, 0.04) - fatigue))
    audio      = max(0.05, min(0.95, 0.70 * humeur + 0.10 * perf_signal     + random.gauss(0, 0.08) - fatigue))
    behavioral = max(0.05, min(0.95, 0.60 * humeur + 0.40 * taux            + random.gauss(0, 0.05) - fatigue))
...
def simuler_session_focalisee(
    ...
    # ── UA principale pour la FK session.cours_id ────────────────────
    ...
    for i, exo in enumerate(exos_session):
        ...
        # ── DÉRIVE DE L'HUMEUR ───────────────────────────────────────
        # L'humeur évolue lentement dans la session, indépendamment de la perf.
        # C'est le signal que l'engagement va capter pour prédire correct[t+1].
        humeur += random.gauss(0, profil["humeur_drift"])
        humeur = max(0.10, min(0.95, humeur))

        # ── PERFORMANCE (dépend de p_mastery ET de l'humeur) ─────────
        p_correct = proba_reussite_avec_humeur(p_mastery, humeur, params_bkt)
        correct = random.random() < p_correct

        # Temps de réponse (plus long si humeur basse, comme dans la vraie vie)
        base_temps = temps_reponse_realiste(p_mastery, exo.difficulte or 1, profil["label"])
        t_sec = int(base_temps * (1.0 + (0.5 - humeur) * 0.5))
        t_sec = max(3, t_sec)
        duree_cumulee += t_sec

        # ── ENGAGEMENT (reflète l'humeur) ────────────────────────────
        taux_recent = (
            sum(correct_recent[-5:]) / max(len(correct_recent[-5:]), 1)
            if correct_recent else None
        )
        eng = simuler_engagement(humeur, correct, taux_recent, i, n_target)
        engagement_scores.append(eng["fused"])
        etats_affectifs.append(eng["etat"])

        # ── MISE À JOUR BKT pour tous les KCs de l'exercice ──────────
        for kc in kcs:
        ...
        # ── ProgressionApprenant (avec engagement par exo) ───────────
...
def simuler_parcours(db, user, profil, params_bkt, exos_par_macro, ua_par_id):
    """Simule N sessions étalées sur 1-3 mois, avec humeur de baseline propre."""
    n_min, n_max = profil["n_sessions"]
    n_sessions = random.randint(n_min, n_max)

    # ── Humeur baseline de l'apprenant ────────────────────────────────
    # Sert de "moyenne" autour de laquelle l'humeur de chaque session fluctue.
    humeur_baseline = _gauss_clamp(*profil["humeur_base"], lo=0.10, hi=0.95)

    # État BKT persistant entre sessions
    bkt_state = {}

    # Distribution des macro-KCs visités sur le parcours :
    # chaque apprenant a 3 macro-KCs "favoris" qu'il visite plus souvent
    ...
    # Poids : favoris 3x plus probables que les autres
    weights = [3.0 if m in macros_favoris else 1.0 for m in macros_disponibles]

    # Première session : entre 60 et 90 jours avant aujourd'hui
    ...
    for s in range(n_sessions):
        # ── Choix des macro-KCs pour CETTE session ────────────────────
        ...
        macros_session = list(set(macros_session))  # déduplique si 2 tirages identiques

        nb_c, nb_t = simuler_session_focalisee(
            db, user, profil, params_bkt,
            macros_session, exos_par_macro, ua_par_id,
            bkt_state, current_dt, humeur_baseline,
        )
        total_correct += nb_c
        total_exos += nb_t

        # Espacement aléatoire entre sessions
