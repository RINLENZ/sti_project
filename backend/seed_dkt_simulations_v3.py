...
PROFILS = [
    ...
    {
        "label":             "volatil",          # *** profil clé pour DKT-E (v3: amplifié) ***
...
def proba_reussite_avec_humeur(p_mastery, humeur, params_bkt):
    ...
    Le P_slip dépend de l'humeur courante (effet AMPLIFIÉ en v3) :
      - humeur = 0.5  →  P_slip nominal
      - humeur = 0.2  →  P_slip + 0.24  (apprenant très distrait → beaucoup d'erreurs)
      - humeur = 0.8  →  P_slip - 0.24  (apprenant concentré → très peu d'erreurs)
    ...
    """
    ajustement = (0.5 - humeur) * 0.80
    p_slip_eff = max(0.01, min(0.70, params_bkt["P_slip"] + ajustement))
