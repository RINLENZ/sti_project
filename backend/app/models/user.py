...
class User(Base):
    ...
    matieres_enseignees = Column(String, nullable=True)
...
class TuteurSuivi(Base):
    ...
    actif        = Column(Boolean, default=True)
    created_at   = Column(DateTime(timezone=True), server_default=func.now())# ← Patch temporaire — sera écrasé par le vrai fichier
