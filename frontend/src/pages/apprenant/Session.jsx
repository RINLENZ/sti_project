    ...
    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 6 }}>
      <div style={{ position: 'relative', width: 90, height: 90 }}>
        ...
        <div style={{ position: 'absolute', inset: 0, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center' }}>
          <span style={{ fontSize: 18, fontWeight: 900, color, lineHeight: 1 }}>{Math.round(score * 100)}</span>
          <span style={{ fontSize: 9, color: C.textSec, fontWeight: 600 }}>%</span>
        </div>
      </div>
      <span style={{ fontSize: 11, fontWeight: 700, color, textAlign: 'center' }}>{engLabel(score)}</span>
    </div>
  )
}
...
export default function Session() {
  ...
  async function startCamera() {
    let attempts = 0
    while ((!window.FaceMesh || !window.Camera) && attempts < 10) { await new Promise(r => setTimeout(r, 500)); attempts++ }
  ...
  function onFaceResults(results) {
    ...
    if (!results.multiFaceLandmarks?.length) {
      ...
      const now = Date.now()
      if (now - lastSendRef.current > 5000) { sendEvent('facial_analysis', { visual_score: 0.0, face_detected: false }); lastSendRef.current = now }
      return
    }
    const lm = results.multiFaceLandmarks[0]
    const ear = (computeEAR(lm,[362,385,387,263,373,380])+computeEAR(lm,[33,160,158,133,153,144]))/2
    const yaw = (lm[1].x-0.5)*180, pitch = (lm[1].y-lm[152].y)*200
    let score = 1.0
    ...
    const now = Date.now()
    if (now-lastSendRef.current>5000) { sendEvent('facial_analysis',{visual_score:Math.round(score*100)/100,ear:Math.round(ear*100)/100,yaw:Math.round(yaw),pitch:Math.round(pitch),face_detected:true}); lastSendRef.current=now }
  }
  ...
  async function soumettre() {
    ...
    try {
      ...
      await sendEvent('response', { exercice_id: ex.id, correct: data.correct, time_seconds: Math.round((Date.now()-startTime)/1000) })
  ...
  return (
    <div style={{ minHeight:'100vh',background:C.bg }}>
      <style>{`@keyframes pulse{0%,100%{opacity:1}50%{opacity:.4}}@keyframes slideDown{from{opacity:0;transform:translateY(-8px)}to{opacity:1;transform:translateY(0)}}@keyframes confettiFall{0%{transform:translateY(-20px) rotate(0deg);opacity:1}100%{transform:translateY(110vh) rotate(720deg);opacity:0}}`}</style>
